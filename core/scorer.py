...
class ConcentrationScorer:
    """The core algorithm that calculates a user's real-time concentration percentage.
    It combines multiple data points (Eye Aspect Ratio, Gaze, Head Pose, AI Object Detection) 
    and applies smoothing and buffer times to prevent false alarms."""
    def __init__(self):
        self.score_history = []
        self.history_length = 30 # Approx 1-2 seconds of data depending on framerate
        
        # Timers used to track how long a user has been violating a rule
    ...
    def _get_graded_score(self, value, low_limit, high_limit, buffer=0.05):
        """Converts a raw measurement into a smooth percentage (0.0 to 1.0).
        Instead of a strict pass/fail, it uses a 'buffer' zone to gradually reduce 
        the score as the user drifts further away from the ideal focus limits."""
        if low_limit <= value <= high_limit: return 1.0
        if value < low_limit:
            diff = low_limit - value
            if diff > buffer: return 0.0
            return 1.0 - (diff / buffer) # Gradual penalty
        if value > high_limit:
    ...
    def get_score(self, ear, gaze_ratio, pitch, yaw, phone_detected=False):
        """Calculates the final concentration score for the current frame."""
        # Blink Score 
        if ear > config.EAR_THRESHOLD:
            score_blink = 1.0
            self.eyes_closed_start_time = None # Reset timer if eyes are open
        else:
            score_blink = 0.0
            # Start timer the exact moment the eyes close
